...
if (isset($_POST['update'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('User information updated successfully.'); window.location.href='former.php';</script>";
    } else {
...
<body>
    <form action="" method="POST">
        ...
        <div class="form-group" style="margin-bottom: 15px;">
            ...
            <select id="role" name="role" style="width: 100%; padding: 8px; border-radius: 4px; border: 1px solid #ddd; box-shadow: none;">
                <option value="">মান নির্বাচন করুন</option>
                <option value="supporter" <?php echo ($role == 'supporter') ? 'selected' : ''; ?>>সমর্থক (supporter)</option>
                <option value="worker" <?php echo ($role == 'worker') ? 'selected' : ''; ?>>কর্মী (worker)</option>
                <option value="partner" <?php echo ($role == 'partner') ? 'selected' : ''; ?>>সাথী (partner)</option>
                <option value="member" <?php echo ($role == 'member') ? 'selected' : ''; ?>>সদস্য (member)</option>
            </select>
        </div>
