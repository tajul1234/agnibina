...
if (isset($_POST['search'])) {
    ...
    $query = "SELECT *  FROM users WHERE approved = 1 AND status = 'active' AND faculty = 'Arts'";
    $stmt = $conn->prepare($query);
}
$stmt->execute();
                ...
                <?php if (!empty($users)): ?>
                    <?php foreach ($users as $user): ?>
                        <tr>
                            ...
                            <td class="name"><a href="artsindividual.php?id=<?php echo $user['id']; ?>"><?php echo htmlspecialchars($user['name']); ?></a></td>
